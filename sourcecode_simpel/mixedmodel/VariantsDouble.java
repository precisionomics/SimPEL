...
//	int[][] location2offset_inblock;
	...
	public static VariantsDouble importCSV(String file_csv,String file_hdf5, int block_size){
		...
		try{
			...
			writer.setBooleanAttribute("/genotype", "transformed", false);
			...
			for(int chr=0;chr<num_chrs;chr++){
				...
				writer.createDoubleMatrix("/genotype/chr"+(1+chr)+"/indi_fast",sample_size, num_sites[chr], sample_size, block_size);
				writer.createDoubleMatrix("/genotype/chr"+(1+chr)+"/position_fast", num_sites[chr], sample_size, block_size, sample_size);
			...
			while(line!=null){
				...
				if(chr!=current_chr){
					...
					writer.writeDoubleMatrixBlockWithOffset("/genotype/chr"+(current_chr)+"/position_fast",
					...
					if(num_of_variants_in_block==block_size){
						...
						writer.writeDoubleMatrixBlock("/genotype/chr"+(current_chr)+"/position_fast",
	...
	public void transform_all_data(RealMatrix transform_matrix, String new_file_hdf5){ 
		...
		try{
			...
			if(hdf5_old.exists()){hdf5_old.delete();System.out.println(new_file_hdf5+" alreday exist. Deleted!");}
			IHDF5Writer writer = HDF5Factory.open(new_file_hdf5);
			// CODE FOR constructing above structure (all groups and tables and importing data)
			writer.createGroup("/genotype");
			writer.setIntAttribute("/genotype", "sample_size", this.sample_size);
			writer.setIntAttribute("/genotype", "block_size", this.block_size);
			writer.setIntAttribute("/genotype", "num_chrs", this.num_chrs);
			writer.writeStringArray("/genotype/sample_ids", this.sample_ids);			
			writer.setBooleanAttribute("/genotype", "transformed", true);
			...
			for(int chr=0;chr<num_chrs;chr++){
				...
				writer.createDoubleMatrix("/genotype/chr"+(1+chr)+"/indi_fast", this.sample_size, this.num_sites[chr], 
						this.sample_size, this.block_size);
				writer.createDoubleMatrix("/genotype/chr"+(1+chr)+"/position_fast", this.num_sites[chr], this.sample_size, 
			...
			for(int chr=0;chr<this.num_chrs;chr++){
				...
				for (HDF5MDDataBlock<MDDoubleArray> block : this.position_fast_blocks[chr]){
					...
					writer.writeDoubleMatrixBlockWithOffset("/genotype/chr"+(chr+1)+"/position_fast",
...
//			System.out.println("THREE");
	...
	public int[][] find_vars_in_maf_range(double min_maf, double max_maf){
		...
		for(int chr=0;chr<this.num_chrs;chr++){
			ArrayList<Integer> the_qualified=new ArrayList<Integer>();
			...
			results[chr]=myFileFunctions.FileFunc.arraylist2array(the_qualified);
		}return results;
	}
	...
	public int[][] find_fixed_num_of_vars_in_maf_range(double min_maf, double max_maf, int num_vars_to_simulate){
		...
		for(int chr=0;chr<this.num_chrs;chr++){			
			the_qualified[chr]=new ArrayList<Integer>();
		}		
		int simulated=0;
...
//				System.out.println(simulated+": chr"+the_chr+"_"+the_location+"_"+maf);
		...
		for(int chr=0;chr<this.num_chrs;chr++){			
			results[chr]=myFileFunctions.FileFunc.arraylist2array(the_qualified[chr]);
		}
		return results;
	}
	...
	public int[] find_vars_in_maf_range_in_region(int chr, int start_location, int end_location, 
		...
		return myFileFunctions.FileFunc.arraylist2array(the_qualified);	 
	}
...
//		this.intercepts=this.reader.readDoubleArray("/genotype/intercepts");
		...
		for(int chr=0;chr<num_chrs;chr++){
			...
			this.position_fast_blocks[chr]=reader.getDoubleMDArrayNaturalBlocks("/genotype/chr"+(1+chr)+"/position_fast");		
			...
			for(int chr=0;chr<this.num_chrs;chr++){				
				for (HDF5MDDataBlock<MDDoubleArray> block : this.position_fast_blocks[chr]){
					...
					for(int var_index=0;var_index<data4thisblock.length;var_index++){
						...
						for(int k=0;k<this.sample_size;k++)new_snp[k]=(data4thisblock[var_index][k]-mean)/sd;
						for(int i=0;i<sample_size;i++){
