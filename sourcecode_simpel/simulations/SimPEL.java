...
public class SimPEL {
	...
	int[] sim_control_indexes2;  // for trios: the sim_control_indexes above will be one parent, and sim_control_indexes2 will be the other	
	...
	public SimPEL(
			...
			int parent_control, int sibling_control, int trios_control,
			// analytic parameters  
					...
					if(!tmp[8].equals("0")){ //there is sibling(s)
						...
						for(int k=0;k<sibs.length;k++){
							if(this.gender.containsKey(sibs[k]) && (!tmp[1].equals(sibs[k])) && this.gender.get(sibs[k]).equals("1")
								...
								this.siblings.put(tmp[1], sibs[k]);  // found a sibling, with gender matched.
								break;
							}
						}
					}
	...
	/*
	 * Simulate case/comntrol samples based on matching options;
	...
	public void simulate_samples(){ // based on the field compond_het, we simulate one or two mutations 
		...
		this.sim_control_indexes2=new int[num_case];
		Arrays.fill(this.sim_control_indexes2, -1); // sim_control_indexes2 is for trio only.
		...
		if(total_trios<this.trios_control){
			...
			for(int i=this.parent_control+this.sibling_control;
				...
				this.sim_control_indexes2[i]=control_index_in_all_samples2;
				used_indexes.add(this.sim_case_indexes[i]);
				used_indexes.add(this.sim_control_indexes[i]);
				used_indexes.add(this.sim_control_indexes2[i]);
			}	
		}		
		...
		Object[] indi_w_siblings=this.siblings.keySet().toArray();
		...
		if(total_siblings<this.sibling_control*2){
			...
			for(int i=this.parent_control;i<this.parent_control+this.sibling_control;i++){
				...
				int case_index_in_all_samples=this.genotype.sample_id2index.get(case_id);
				String control_id=this.siblings.get(indi_w_siblings[index]);
				int control_index_in_all_samples=this.genotype.sample_id2index.get(control_id);
				while(index==total_siblings || used_indexes.contains(case_index_in_all_samples) 
					...
					case_index_in_all_samples=this.genotype.sample_id2index.get(case_id);
					control_id=this.siblings.get(indi_w_siblings[index]);
					control_index_in_all_samples=this.genotype.sample_id2index.get(control_id);
				}											
				this.sim_case_indexes[i]=case_index_in_all_samples;
				this.sim_control_indexes[i]=control_index_in_all_samples;
		...
		if(genotype.sample_size<this.num_case*2){
			...
			for(int i=this.parent_control+this.sibling_control+this.trios_control;i<num_case;i++){
				//select a case
		...
		for(int case_index=0;case_index<num_case;case_index++){
			...
			if(Test.randomNumber()>=control_random_cut){ // contribute to the scores if the control doesn't have that mutation
				...
				if(this.sim_control_indexes2[case_index]==-1){
				...
				}else{ // both parents are present. so need to check the other parent too. 
	...
	public int in_family(int index, int index2){
		...
		else if((this.siblings.containsKey(id)&&this.siblings.get(id).contains(id2))
				||(this.siblings.containsKey(id2)&&this.siblings.get(id2).contains(id)))return 1;
		else return 0;		
	}
	 ...
	 */
	public int[] analysis_candidate_genes_unknown(){
		double[] standings=new double[heterogeneity];
		int[] int_standings=new int[this.heterogeneity];
		Arrays.fill(standings, Double.NaN);
		...
		try{
				...
				for(int i=0;i<genotype.locations[chr].length;i++){
					...
					if(loc2gene_index[loc]!=-1){// found the gene the loc belongs!
						...
						for(int k=0;k<num_case;k++){
							...
							int control_index2=this.sim_control_indexes2[k];
							if(control_index2==-1){ // normal control: no trios
			...
			for(int hg=0;hg<heterogeneity;hg++){
				...
				standings[hg]=Arrays.binarySearch(the_gene_score, causal_real_scores[hg]); 
				if(Double.isNaN(standings[hg])){
					int_standings[hg]= -100;	continue;
				}
				if(standings[hg]<0)standings[hg]=this.all_genes.length+standings[hg]; // not found, convert the index to the positive one
				else if(standings[hg]>=0)standings[hg]=this.all_genes.length-standings[hg];
				int_standings[hg]=(int)(standings[hg]+0.001); // ensure the machine error corrected
				System.out.println("Standings of the #"+hg+" gene: "+(int_standings[hg]+1)+"/"+this.all_genes.length);					
			}					
		}catch(Exception e){e.printStackTrace();}
		return int_standings;  
	}
	
	/*
	 * Analysis when candidate genes are unknown 
	 */
	public int[] analysis_candidate_genes_known(){
		double[] standings=new double[heterogeneity];
		int[] int_standings=new int[this.heterogeneity];
		Arrays.fill(standings, Double.NaN);
		...
		try{
			...
			for(int hg=0;hg<heterogeneity;hg++){
				...
				if(standings[hg]<0)standings[hg]=this.candidate_genes.length+standings[hg]; // not found, convert the index to the positive one
				else if(standings[hg]>=0)standings[hg]=this.candidate_genes.length-standings[hg];
				int_standings[hg]=(int)(standings[hg]+0.001); // ensure the machine error corrected
				System.out.println("Standings of the #"+hg+" gene: "+(int_standings[hg]+1)+"/"+this.candidate_genes.length);	
				
			}		
		}catch(Exception e){e.printStackTrace();}
		return int_standings;  // ensure the machine error corrected
	}
	
	public void test_power(int round, int num_top_gene, String report_file){
		int[][] standings=new int[round][];
		...
		try{
			...
			for(int r=0;r<round;r++){
				...
				for(int k=0;k<num_case;k++){
					...
					if(this.sim_control_indexes2[k]!=-1)
						bw.write("#\tCtrl:\t"+genotype.sample_ids[this.sim_control_indexes2[k]]+"/"
							+this.id2ethnicity.get(genotype.sample_ids[this.sim_control_indexes2[k]])+"\n");
...
//						bw.write("NA\n");
	...
	public static void main(String[] args) {
		...
		String report_file=folder+"report.known.t2.h2.txt";
		...
		SimPEL ap=new SimPEL(gencode_file, candidate_genes_file1, ethnicity_file, KG_hdf5, ExAC_file, 
				...
				parent_control,  sibling_control, trios_control, individual_weight, ann_score_cutoff, 
				ann_score_confidence, genes_file_known, compound_het, tmp_working);
